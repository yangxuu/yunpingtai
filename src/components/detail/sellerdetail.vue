<template>
  <div>
    ...
    <div class="bread-nav safe auto">
      ...
      <span>当前位置：</span>
      <span v-for="(item,index) in info.result.nav" :key="index">{{item.name}}></span>
      <span>{{info.result.detail.name}}</span>
    </div>
    ...
    <div class="col-top-box safe auto clearfix">
      <div class="col-left" id="big">
        <div class="banner-box" id="pic" @mousemove="move($event)" @mouseout="out">
          <img
            v-for="(item,index) in info.result.detail.thumb"
            v-show="index==num"
            :key="index"
            :src="'http://files.cailiao.com/'+item.image"
            alt
          />
        </div>
      ...
      <div class="col-medium">
        <div class="goodsname">{{info.result.detail.name}}</div>
        ...
        <div class="col-m-row2">
          <div class="clearfix" style="height: 75px;">
            <div class="row2-left">
              ...
              <span class="row2-val">
                ￥
                <i>{{info.result.detail.price_description}}</i>
              </span>
            </div>
            ...
            <div class="row2-left">
              ...
              <span class="row2-val">
                <i>{{info.result.detail.amount}}</i> {{info.result.detail.unit}}
              </span>
            </div>
          </div>
        </div>
        ...
        <div class="col-m-row4 clearfix">
          ...
          <div class="row4-val">{{info.result.detail.enterprise_name}}</div>
        </div>
        <div class="col-m-row4 clearfix">
          ...
          <div class="row4-val">
            ...
            >{{info.result.detail.contact_mobile}} / {{info.result.detail.phone}}</span>
            <span class="row4-linkman">&#x3000;{{info.result.detail.link_manage}}</span>
          </div>
        </div>
        <div class="col-m-row4 clearfix addborder">
          <div class="row4-key">所在区域</div>
          <div class="row4-val">{{info.result.detail.business_address}}</div>
        </div>
      ...
      <!-- <div class="col-right">
        ...
        <div class="col-r-row2">
          <router-link
            ...
            v-for="(item,index) in info.result.newest_supply.data"
            :key="index"
          >
            <div class="img-box">
              <img :src="'http://files.cailiao.com/'+item.image" alt />
        ...
        <div class="products-box">
          <div class="boss-goods" ref="margin">
            <router-link
              ...
              v-for="(item,index) in info.result.newest_seller.data"
              :key="index"
            >
              <div class="img-box">
                <img :src="'http://files.cailiao.com/'+item.image" alt />
    ...
    <div class="col-detail safe auto clearfix">
      <div class="detail-left">
        <div
          class="shop-level"
        >{{info.result.detail.user_grade==1?"普通会员":info.result.detail.user_grade==2?"标准会员":info.result.detail.user_grade==3?"超级会员":info.result.detail.user_grade==4?"至尊会员":""}}</div>
        <div class="shop-name">{{info.result.detail.enterprise_name}}</div>
        <div class="shop-object">
          ...
          <div class="object-row">
            ...
            <span class="object-val">{{info.result.detail.link_manage}}</span>
          </div>
          <div class="object-row m-t-15">
            <span class="object-key">手机号码：</span>
            <span class="object-val">{{info.result.detail.contact_mobile}}</span>
          </div>
          <div class="object-row m-t-15">
            <span class="object-key">电话号码：</span>
            <span class="object-val">{{info.result.detail.phone}}</span>
          </div>
          <div class="object-row m-t-15">
            <span class="object-key">传真：</span>
            <span class="object-val">{{info.result.detail.facsimile}}</span>
          </div>
          <div class="object-row m-t-15">
            <span class="object-key">主营产品：</span>
            <span class="object-val">{{info.result.detail.product}}</span>
          </div>
        </div>
        ...
        <div class="kinds-box">
          ...
          <div class="kinds-list">
            <ul id="menu">
              <li class="firstLevel" v-for="(one,index) in info.result.cate" :key="index">
...
export default {
  ...
  created() {
    ...
    // 供应库详情
    var data = {
      action: "cailiaof.seller.sellerdetail",
  ...
  methods: {
    ...
    next() {
      this.num++;
      if (this.num == this.info.result.detail.thumb.length) {
        this.num = 0;
      }
    },
    // 轮播上一张
    prev() {
      if (this.num <= 0) {
        this.num = this.info.result.detail.thumb.length;
      }
      this.num--;
    },
    // 放大镜移入
